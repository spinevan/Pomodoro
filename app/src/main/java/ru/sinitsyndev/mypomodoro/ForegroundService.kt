...
class ForegroundService : Service() {
    ...
    private fun processCommand(intent: Intent?) {
        when (intent?.extras?.getString(COMMAND_ID) ?: INVALID) {
            COMMAND_START -> {
                val startedStopwatch = intent?.extras?.getParcelable<Stopwatch>(STARTED_TIMER_OBJECT) ?: return
                commandStart(startedStopwatch)
            }
    ...
    private fun commandStart(stopwatch: Stopwatch) {
        if (isServiceStarted) {
            return
        }
    ...
    private fun continueTimer(stopwatch: Stopwatch) {
       var timeToStop: Long = stopwatch.currentSec
